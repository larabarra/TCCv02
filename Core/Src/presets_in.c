...
#define _MPU6050_ADDR   0x68

HAL_StatusTypeDef MPU6050_Init_I2C6050(void)
{
    uint8_t check = 0;
    uint8_t data  = 0;
...
HAL_StatusTypeDef MPU6050_Read_Accel_I2C6050(int16_t* ax, int16_t* ay, int16_t* az, float* Ax_g, float* Ay_g, float* Az_g)
{
    uint8_t r[6];
...
HAL_StatusTypeDef MPU6050_Read_Gyro_I2C6050(int16_t* gx, int16_t* gy, int16_t* gz, float* Gx_dps, float* Gy_dps, float* Gz_dps)
{
    uint8_t r[6];
...
void MPU6050_Read_Accel(float *ax, float *ay, float *az)
{
    ...
    if (MPU6050_Read_Accel_I2C6050(&ax_raw, &ay_raw, &az_raw, &ax_g, &ay_g, &az_g) == HAL_OK)
    {
        if (ax) *ax = ax_g;
        if (ay) *ay = ay_g;
        if (az) *az = az_g;
...
void MPU6050_Read_Gyro(float *gx, float *gy, float *gz)
{
    ...
    if (MPU6050_Read_Gyro_I2C6050(&gx_raw, &gy_raw, &gz_raw, &gx_dps, &gy_dps, &gz_dps) == HAL_OK)
    {
        if (gx) *gx = gx_dps;
        if (gy) *gy = gy_dps;
        if (gz) *gz = gz_dps;
