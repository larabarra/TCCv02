...
static void lcd_send_cmd(uint8_t cmd)
{
    ...
    HAL_I2C_Master_Transmit(&{{ i2c_handle }}, LCD_ADDR, t, 4, 100);
}
...
HAL_StatusTypeDef OUT_UART_Print(const char *s)
{
    ...
    return HAL_UART_Transmit(&{{ uart_handle }}, (uint8_t*)p, (uint16_t)strlen(s), 100);
}
{% endif %}
...
void PWM_Set(uint16_t duty_0_1000)
{
    ...
    __HAL_TIM_SET_COMPARE(&{{ tim_handle }}, TIM_CHANNEL_1, duty_0_1000);
}
{% endif %}
